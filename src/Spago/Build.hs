...
import qualified Spago.Config.Legacy as LegacyConfig
...
build :: HasBuildEnv env => Maybe (RIO BuildEnv ()) -> RIO env ()
build maybePostBuild = do
  logDebug "Running `spago build`"
  BuildOptions{..} <- view (the @BuildOptions)
  Config{..} <- view (the @Config)
  (_targetName, Target{..}) <- view (the @BuildTarget)
  deps <- Packages.getTargetTransitiveDeps
  let partitionedGlobs@Packages.Globs{..} = Packages.getGlobs deps depsOnly (toList targetSourcePaths)
      allPsGlobs = Packages.getGlobsSourcePaths partitionedGlobs <> sourcePaths
      allJsGlobs = Packages.getJsGlobs deps depsOnly (toList targetSourcePaths) <> sourcePaths
      ...
      checkImports = do
        ...
        for_ maybeGraph $ \(Purs.ModuleGraph moduleGraph) -> do
              let
                ...
                dependencyPackages = Set.fromList $ Map.keys targetDependencies
...
repl newPackages sourcePaths pursArgs depsOnly replPackage = do
  ...
  Config.ensureConfig >>= \case
    Right config@Config{ targets } -> do
      target <- Run.getTarget targets
      Run.withReplEnv config (fmap ensurePsciSupportIncluded target) (replAction purs)
    Left err -> do
      logDebug err
      ...
      Temp.withTempDirectory cacheDir "spago-repl-tmp" $ \dir -> do
        ...
        legacyConfig <- Run.withPursEnv NoPsa $ do
          LegacyConfig.makeTempConfig dependencies Nothing Set.empty Nothing
        let config@Config{targets} = Config.fromLegacy legacyConfig
        target <- case headMay (Map.toList targets) of
          Nothing -> die [ "Internal error: could not find autogenerated target" ]
          Just t -> pure t

        Run.withReplEnv config (fmap ensurePsciSupportIncluded target) (replAction purs)
  where
    ensurePsciSupportIncluded :: Target -> Target
    ensurePsciSupportIncluded originalTarget@Target{ targetDependencies = originalDeps} =
      case Map.lookup replPackage originalDeps of
        Just _ -> originalTarget
        Nothing -> originalTarget { targetDependencies = Map.insert replPackage "*" originalDeps }

    replAction purs = do
      (_targetName, Target { targetSourcePaths }) <- view (the @BuildTarget)
      deps <- Packages.getTargetTransitiveDeps
      let
        globs =
          Packages.getGlobsSourcePaths $ Packages.getGlobs deps depsOnly (toList $ targetSourcePaths <> Set.fromList sourcePaths)
...
test maybeModuleName extraArgs = do
  ...
  Config { alternateBackend } <- view (the @Config)
...
script modulePath tag packageDeps opts = do
  ...
  legacyConfig <- Run.withPursEnv NoPsa $ do
    LegacyConfig.makeTempConfig dependencies Nothing (Set.fromList [ SourcePath absoluteModulePath ]) tag
  let config@Config{ targets } = Config.fromLegacy legacyConfig
  -- TODO: do we need this _target?
  _target <- case headMay (Map.toList targets) of
    Nothing -> die [ "Internal error: could not find autogenerated target" ]
    Just t -> pure t
...
bundleApp withMain maybeModuleName maybeTargetPath noBuild buildOpts usePsa =
  ...
  in Run.withBuildEnv usePsa buildOpts $ case noBuild of
    DoBuild -> build (Just bundleAction)
...
bundleModule maybeModuleName maybeTargetPath noBuild buildOpts usePsa = do
  ...
  Run.withBuildEnv usePsa buildOpts $ case noBuild of
    DoBuild -> build (Just bundleAction)
    NoBuild -> bundleAction
...
docs format noSearch open = do
  ...
  Purs.docs docsFormat $ Packages.getGlobsSourcePaths (Packages.getGlobs deps depsOnly (toList targetSourcePaths)) <> sourcePaths
  ...
  when isHTMLFormat $ do
    when (noSearch == AddSearch) $ do
      ...
      let cmd = "node .spago/purescript-docs-search build-index --package-name " <> surroundQuote targetName
...
search = do
  ...
  Purs.compile (Packages.getGlobsSourcePaths (Packages.getGlobs deps Packages.AllSources (toList targetSourcePaths)))
    [ PursArg "--codegen"
    , PursArg "docs"
    ]

  writeTextFile ".spago/purescript-docs-search" =<< docsSearchTemplate
  let cmd = "node .spago/purescript-docs-search search --package-name " <> surroundQuote targetName
  logDebug $ "Running `" <> display cmd <> "`"
