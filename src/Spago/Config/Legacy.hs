...
import           Spago.Env hiding (Config, Repo)
...
parsePackage (Dhall.RecordLit ks') = do
  ...
  let repoFromRepo (Repo repo) = ConfigV1.Git { subdir = Nothing, url = repo }
  repo <- repoFromRepo <$> Dhall.requireTypedKey ks "repo" (Dhall.auto :: Dhall.Decoder Repo)
  ref <- Dhall.requireTypedKey ks "version" Dhall.strictText
...
parsePackageSet pkgs = do
  ...
  let packagesMinPursVersion = join
        $ fmap (hush . Version.semver . Text.replace "v" "" . ref . location)
      ...
      case SemVer.parseSemVerRange unparsedRange of
        ...
        Right range -> case Text.stripPrefix "purescript-" name of
          ...
          Just packageSetName | package <- PackageName packageSetName -> case Map.lookup package packagesDB of
            ...
            Just Package{ location = Remote {..} } -> case SemVer.parseSemVer ref of
              Right v | SemVer.matches range v -> Right package
              _                                -> Left $ WrongVersion package range ref
